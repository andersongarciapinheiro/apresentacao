...
function Performance_Align(){
	...
	this.automateTextTelecine = automateTextTelecine;
	...
	function automateTextTelecine(elemento,limite,all,modo){
		var tamanhoDinamico = window.getComputedStyle(document.querySelector(elemento), null).getPropertyValue('width').split("px"); 
	    ...
	    if(all===true){
	    	if(modo === "largura"){
		        ...
		        if(tamanhoString>limite){
		            fontSizeA = fontSizeA - 1;
		            el(elemento).style.fontSize = fontSizeA+'px';
		            el(elemento).style.lineHeight = fontSizeA+4+'px';
		            // console.log("AKI "+fontSizeA+" TAMANHO "+div.clientWidth);
		            document.getElementById("banner").removeChild(div);
		            automateTextTelecine(elemento,limite,all,modo);
		    ...
		    for(var i = 0 ; i < explodeTxtAuto1.length ; i++){
		            ...
		            if(tamanhoString>limite){
		                fontSizeA = fontSizeA - 1;
						el(elemento).style.fontSize = fontSizeA+'px';
						el(elemento).style.lineHeight = fontSizeA+'px';
		                // console.log("AKI "+fontSizeA+" TAMANHO "+div.clientWidth);
		                document.getElementById("banner").removeChild(div);
		                automateTextTelecine(elemento,limite,all,modo);
		                break;
		...
		setTimeout(function(){
			if(count<steps){
				...
				if(controle_loop){
					...
					spriteScript(elemento,posicao,largura,0,steps,timer,atualizando_Looping);
				}
			}
		},spriteTimer);
	}
